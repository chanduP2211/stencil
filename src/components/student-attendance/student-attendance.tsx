...
export class StudentAttendance {
          ...
          <div class="bar">
            <select class="input" ref={data => (this.section = data)}>
              ...
              {this.details.map(element => {
                return <option value={element.section}>{element.section}</option>;
              })}
            </select>
            <input class="input" type="date" placeholder="Add Date" ref={data => (this.date = data)} />
            <button class="input" onClick={this.getting.bind(this)}>
              GET
            </button>
          </div>
        </div>
        <div class="content">
          <form name="form" id="form">
            <div class="table">
              ...
              <div class="element3">
                <button class="submit" onClick={this.submiting.bind(this)}>
                  SUBMIT
                </button>
              </div>
            </div>
  ...
  async getSubject() {
    let res: any[] = await Data.api('getLoginTeacher', 'GET', '');
    let teacher: any[] = await Data.api('getTeacher', 'GET', '');
    let tname: string = '';
    res.map(element => {
      teacher.map(person => {
        if (element.id === person.id) tname = person.name;
      });
    });
    let respone: any[] = await Data.api('getSubject', 'GET', '');
    let sub: string = '';
    respone.forEach(element => {
      if (tname === element.faculty_name) sub = element.subject_name;
    });
    return sub;
  }
}
