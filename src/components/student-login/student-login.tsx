...
async componentWillLoad(){
  ...
  render(){
    ...
    if(this.check)
     content1 = (
       <p id='user'>INVALID USERNAME</p>
     )
     let content2 = ''
     if(this.test)
      content2 = (
        <p id='user'>INVALID PASSWORD</p>
      )

      return(
        <div class='main'> 
        <div class='header'>
            <div class='head1'><h1>STUDENT ATTENDANCE MANAGEMENT SYSTEM</h1></div>
         ...
         <form onSubmit={this.submiting.bind(this)}>
            ...
            {content1}
             ...
             <input type="password" id="password"  onInput={this.checkPassword.bind(this)}/><br />
             {content2}
         </div>
...
submiting(event: Event){
      ...
      if(!this.test && !this.check && (username.value !=''||password.value !=''))
        this.valid = true
      else
        this.valid = false
